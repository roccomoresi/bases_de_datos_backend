...
public interface MedicionRepository extends CassandraRepository<Medicion, UUID> {
    ...
    @Query("SELECT * FROM mediciones_por_sensor WHERE sensor_id = :sensorId AND fecha_medicion >= :desde AND fecha_medicion <= :hasta")
    List<Medicion> findBySensorIdAndFechaBetween(UUID sensorId, java.util.Date desde, java.util.Date hasta);
